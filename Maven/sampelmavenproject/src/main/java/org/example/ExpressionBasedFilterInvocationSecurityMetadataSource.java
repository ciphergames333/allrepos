...
import org.springframework.security.core.annotation.SecuredAnnotationSecurityMetadataSource;
...
public final class ExpressionBasedFilterInvocationSecurityMetadataSource
	...
	private static final Log log = LogFactory.getLog(ExpressionBasedFilterInvocationSecurityMetadataSource.class);
	...
	public ExpressionBasedFilterInvocationSecurityMetadataSource(
		...
		Assert.notNull(expressionHandler, "SecurityExpressionHandler cannot be null");
	}
	...
	private static LinkedHashMap<RequestMatcher, Collection<ConfigAttribute>> processMap(
		...
		requestMap.forEach((requestMatcher, value) -> process(parser, requestMatcher, value, processed::put));
		return processed;
	}

	private static void process(ExpressionParser parser, RequestMatcher requestMatcher, Collection<ConfigAttribute> value,
			BiConsumer<RequestMatcher, Collection<ConfigAttribute>> consumer) {
		String expression = getExpression(requestMatcher, value);
		if (log.isDebugEnabled()) {
			log.debug(LogMessage.format("Adding web access control expression [%s] for %s", expression, requestMatcher));
		}
		AbstractVariableEvaluationContextPostProcessor postProcessor = createPostProcessor(requestMatcher);
		...
		consumer.accept(requestMatcher, processed);
	}

	private static String getExpression(RequestMatcher requestMatcher, Collection<ConfigAttribute> value) {
		Assert.isTrue(value.size() == 1, () -> "Expected a single expression attribute for " + requestMatcher);
	...
	static class RequestVariablesExtractorEvaluationContextPostProcessor
		...
		Map<String, String> extractVariables(HttpServletRequest request) {
			return this.requestMatcher.matcher(request).getVariables();
		}
